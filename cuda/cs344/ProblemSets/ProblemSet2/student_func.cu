                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  // to sequential reference solution for the exact clamping semantics you should follow.
    ...
    int input_offset = h_i * numCols + w_i;

    if ( w_i >= numCols || h_i >= numRows)
    {
      return;
    }

    // first threads in every block copy the filter into shared memory
    // dynamic alloc shared memory
    // <<<Grid, Block, SharedMemSize>>>
    extern __shared__ float sfilter[];

    if (DEBUG && w_i==0 && h_i==0)
    {
      printf("filterWidth  %d \n", filterWidth);
    }
      

    // use few threads to initialize the shared memory
    ...
    // if ( DEBUG && w_i == 0 && h_i ==0)
    // {
    //   for( int row_delta=0; row_delta<filterWidth; row_delta++){
    //     for(int col_delta=0; col_delta<filterWidth; col_delta++){
    //       int offset = row_delta * filterWidth + col_delta;
    //       float value = sfilter[offset];
    //       printf("%f ", value);
    //     }
    //     printf("\n");
    //   }
    ...
    // if filter_width=3, row:-1,0,1, col:-1,0,1
    float result = 0.0f;
    ...
    for( int row_delta=-filterWidth/2; row_delta<=filterWidth/2; row_delta++){
      for(int col_delta=-filterWidth/2; col_delta<=filterWidth/2; col_delta++){
        ...
        int value_row = min(max(h_i + row_delta, 0), numRows - 1);
        int value_col = min(max(w_i + col_delta, 0), numCols - 1);

        // Compute the partial sum this value adds to the result when scaled by the 
        // appropriate coefficient.
        int img_offset = value_row * numCols + value_col;
        int filter_offset = (row_delta + filterWidth / 2) * filterWidth + (col_delta + filterWidth / 2);

        float channel_value = static_cast<float>(inputChannel[img_offset]);
        float filter_coefficient = sfilter[filter_offset];

        result = result + channel_value * filter_coefficient;
      }
    }

    outputChannel[input_offset] = result;
                      ...
                      unsigned char* const blueChannel)
{
  ...
  // Compute the thread's row and col
  int col = blockIdx.x * blockDim.x + threadIdx.x; // w
  int row = blockIdx.y * blockDim.y + threadIdx.y; // h
  ...
  {
    printf("At offset %d, Red %u, Green %u, Blue %u", offset, 
    redChannel[offset], greenChannel[offset], blueChannel[offset]);
  }
                        ...
                        const int filterWidth)
{
  ...
  if DEBUG
  {
    size_t num_pixels = numCols * numRows;
    printf("Image Size, Width: %d, Height %d, Pixels %d\n", numCols, numRows, num_pixels);
    printf("Block Size, x:%d, y:%d, z:%d\n", blockSize.x, blockSize.y, blockSize.z);
    printf("Gird Size, x:%d, y:%d, z:%d\n", gridSize.x, gridSize.y, gridSize.z);
  }
  ...
  timer.Stop();
  std::cout<< "Seperate Channel elapsed: " << timer.Elapsed() << "ms\n";

  timer.Start();

  int shared_size = filterWidth * filterWidth * sizeof(*d_filter);
  gaussian_blur<<<gridSize, blockSize, shared_size>>>(
  ...
  timer.Stop();
  std::cout<< "Gaussion blur x3 elapsed: " << timer.Elapsed() << "ms\n";
  timer.Start();
  ...
  timer.Stop();
  std::cout<< "Combine Channel elapsed: " << timer.Elapsed() << "ms\n";
